...
var CompCharts = function() {
    ...
    return {
        init: function() {
            ...
            var bar = {show: true, lineWidth: 0, fillColor: {colors: [{opacity: 0.5}, {opacity: 0.5}]}};
            
            function getSalarioMinimo(item) {
            	var puesto  = new Object();;
            	puesto.label = item.nombre;
            	puesto.data = item.salarioMinimo;
            	puesto.bars = bar;
            		console.log(item);
            	return puesto;
            }
            
            function getSalarioMaximo(item) {
              	var puesto = new Object();
            	puesto.label = item.nombre;
            	puesto.data = item.salarioMaximo;
            	puesto.bars= bar;
            		console.log(item);
            	return puesto;
            ...
            var dataTest = [{label: 'Tecnologia', data: 30}, {label: 'Contabilidad', data: 20}, {label: 'Ventas', data: 250}, {label: 'Compras', data: 300}];

            // Array with month labels used in Classic and Stacked chart
            var chartMonths = [[1, 'Tecnologia'], [2, 'Contabilidad'], [3, 'Ventas'], [4, 'Compras']];
...
//            );
            ...
            $.get("http://localhost:8088/puestos/dashboard", 
            		function(response)
            		{
     					...
     					var salariosMinimos = response.map(getSalarioMinimo);
 						console.log(salariosMinimos);
 							puestosSalarios.push.apply(puestosSalarios,salariosMinimos);
 							var salariosMaximos = response.map(getSalarioMaximo);
 							puestosSalarios.push.apply(puestosSalarios,salariosMaximos);
 							console.log(puestosSalarios);	
 
                            
                            
                            
                            // Bars Chart
                            $.plot(chartBars, salariosMinimos,config);
                            
                            // Bars Chart
                            $.plot(chartClassic, salariosMaximos,config);
                        });
